// src/components/patient/ExpensesPage.js
...
function ExpensesPage({ patientInfo }) {
  ...
  useEffect(() => {
    // Fetch prescriptions & appointments from your backend here
  }, [patientInfo]);

  const calculateMedicineCost = () => prescriptions.reduce((total, p) => total + (p.days * p.cost_per_day), 0);
  const calculateConsultationCost = () => appointments.reduce((total, a) => total + (a.consultation_fee || 0), 0);
  const calculateTotalExpense = () => calculateMedicineCost() + calculateConsultationCost();
